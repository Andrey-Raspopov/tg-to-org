...
class TextFormatter(BaseFormatter):
    ...
    def generate_message(self, message):
        ...
        who = self.get_display_name(self.get_user(message.from_id)) or UNKNOWN_USER_TEXT
        ...
        if message.reply_message is not None:
            if message.reply_message == ():
                ...
                reply_sender = (
                    self.get_display_name(message.reply_message.from_user)
                    or UNKNOWN_USER_TEXT
                )
                reply_text = message.reply_message.text or ""
                reply = ' (in reply to {}\'s: "{}")'.format(reply_sender, reply_text)
        else:
            reply = ""
    ...
    def _format(self, context_id, file, *args, **kwargs):
        ...
        for message in self.get_messages_from_context(context_id, order="ASC"):
