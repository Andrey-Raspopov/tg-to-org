    ...
    for entity in entities:
        if entity.__class__ in ENTITY_TO_TEXT:
            if isinstance(entity, types.MessageEntityTextUrl):
                extra = ",{}".format(entity.url.replace(",", "%2c").replace(";", "%3b"))
            ...
            parsed.append(
                "{},{},{}{}".format(
                    ENTITY_TO_TEXT[type(entity)], entity.offset, entity.length, extra
                )
            )
    return ";".join(parsed)
...
def decode_msg_entities(string):
    ...
    for part in string.split(";"):
        ...
        if kind in TEXT_TO_ENTITY:
            if kind == "texturl":
                parsed.append(types.MessageEntityTextUrl(offset, length, split[-1]))
            elif kind == "mentionname":
                parsed.append(
                    types.MessageEntityMentionName(offset, length, int(split[-1]))
                )
            else:
...
def get_media_type(media):
    ...
    if isinstance(media, (types.Photo, types.UserProfilePhoto, types.ChatPhoto)):
        return "chatphoto"
...
def get_extension(mime):
    ...
    return (
        COMMON_MIME_TO_EXTENSION.get(mime) or mimetypes.guess_extension(mime) or ".bin"
    )
...
def action_to_name(action):
    """
    Returns a namespaced "friendly" name for the given ``MessageAction``.
    """
    return {
